  ...
  const handleDownload = async (format: 'xlsx' | 'csv') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Determine the file extension based on whether it's a ZIP or single file
        const isZip = stats?.outputFormat === 'zip'
        const extension = isZip ? 'zip' : format

        a.download = `filtered_phone_numbers_${format}.${extension}`
      ...
      <Card className="text-center">
        <CardContent className="p-8">
          ...
          <div className="space-y-4 mb-8">
            {stats.outputFormat === 'zip' ? (
              // Show ZIP download options when files are split
              <>
                <Button
                ...
                >
                  ...
                  {downloading ? 'Preparing Download...' : 'Download Excel Files (ZIP)'}
                </Button>
              ...
              // Show regular download options for single files
              <>
                <Button
                  onClick={() => handleDownload('xlsx')}
                  disabled={downloading}
          ...
          <div className="bg-blue-50 rounded-lg p-4 mb-6">
            ...
            <div className="text-sm text-blue-700 space-y-1">
              {stats.outputFormat === 'zip' ? (
                <>
                  <p>• Files are split into chunks of 50,000 numbers each</p>
                  <p>• ZIP archives contain multiple files for easier handling</p>
                  <p>• Choose CSV or Excel format based on your needs</p>
                ...
                <>
                  ...
                  <p>• Output contains only the &apos;telefon&apos; column</p>
                  <p>• Processing completed in {stats.processingTime}s</p>
                </>
              )}
            </div>
